...
\author{Lại Tiến Đệ, Phạm Xuân Nghĩa, Trần Đức Trung}
\institute{REV-JOURNAL, 05/2026}
\date{}
...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    {\normalsize Lại Tiến Đệ, Phạm Xuân Nghĩa, Trần Đức Trung}\\[1mm]
    {\small REV-JOURNAL, tháng 5/2026}
  \end{center}
\end{frame}
...
% 5
\begin{frame}{Mạch Phân Tích Và Triển Khai}
  \begin{center}
  \begin{tikzpicture}[font=\small, node distance=6mm]
    \node[draw,rounded corners,fill=MtaSoft,minimum width=30mm,minimum height=9mm] (p1) {Đặt vấn đề};
    \node[draw,rounded corners,fill=MtaSoft,right=of p1,minimum width=30mm,minimum height=9mm] (p2) {Mô hình kênh};
    \node[draw,rounded corners,fill=MtaSoft,right=of p2,minimum width=30mm,minimum height=9mm] (p3) {Kiến trúc AE};
    \node[draw,rounded corners,fill=MtaSoft,below=of p3,minimum width=30mm,minimum height=9mm] (p4) {Training};
    \node[draw,rounded corners,fill=MtaSoft,left=of p4,minimum width=30mm,minimum height=9mm] (p5) {Kết quả BER};
    \node[draw,rounded corners,fill=MtaGold!20,left=of p5,minimum width=30mm,minimum height=9mm] (p6) {Kết luận};
    \draw[-{Stealth},thick,MtaTeal] (p1)--(p2)--(p3)--(p4)--(p5)--(p6);
  \end{tikzpicture}
  \end{center}
  \vspace{2mm}
  \begin{block}{Nguyên tắc trình bày}
  Nội dung được triển khai từ giới hạn của hệ thống khối cổ điển, sau đó xây dựng mô hình kênh không lý tưởng, đề xuất ba kiến trúc AutoEncoder và phân tích kết quả mô phỏng theo từng kịch bản.
  \end{block}
\end{frame}
...
\begin{frame}[plain]
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.58\textwidth}
      ...
      \end{block}
      \vspace{2mm}
      {\small Lại Tiến Đệ, Phạm Xuân Nghĩa, Trần Đức Trung\\
      REV-JOURNAL, tháng 5/2026}
    \end{column}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      \begin{tikzpicture}[font=\scriptsize, node distance=3mm]
        \node[draw,rounded corners,fill=MtaSoft,minimum width=32mm,minimum height=7mm] (qam) {16-QAM cố định};
        \node[draw,rounded corners,fill=MtaSoft,below=of qam,minimum width=32mm,minimum height=7mm] (pa) {PA phi tuyến};
        \node[draw,rounded corners,fill=MtaSoft,below=of pa,minimum width=32mm,minimum height=7mm] (cfo) {CFO};
        \node[draw,rounded corners,fill=MtaSoft,below=of cfo,minimum width=32mm,minimum height=7mm] (eq) {ZF/PLL/MMSE};
        \node[draw,rounded corners,fill=MtaGold!18,below=of eq,minimum width=32mm,minimum height=7mm] (ber) {Error floor};
        \draw[-{Stealth},thick,MtaTeal] (qam)--(pa)--(cfo)--(eq)--(ber);
      \end{tikzpicture}
      \vspace{0.5mm}
      \begin{block}{Vấn đề cốt lõi}
      Tối ưu từng khối riêng lẻ không bảo đảm tối ưu toàn tuyến khi nhiều suy hao cùng tồn tại.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
