\subsection{Soft-Output Neural Demodulation Requirements}
...
A soft-output neural Demodulator can be trained with the following bit-wise binary cross-entropy objective:
\begin{equation}
...
\end{equation}
This equation is the neural analogue of classical iterative Demodulation. It shows how AutoEncoder ideas can be connected to BIBCM-ID without overstating the results. AutoEncoder Modulation is used here as a supporting study, but the mathematical interface for future soft-output Demodulation is already clear.

\subsection{Learned Demapper Architectures and Engineering Trade-Offs}
...
\subsection{Relevance to LDPC Decoder Input Reliability}
The title emphasizes ANN utilization in LDPC decoding for BIBCM-ID systems. A narrow interpretation would include only the channel decoder. However, BIBCM-ID is an iterative receiver architecture, and the LDPC decoder is driven by soft information produced by Demodulation. Therefore, a technically complete treatment should explain not only the decoder update but also the nature of the input messages that the decoder receives. Chapter 2 serves this purpose.

The direct contribution remains the ANN-assisted check-node update in LDPC decoding. The AutoEncoder material supports the system context by analyzing how learned Modulation and Demodulation can affect the reliability of the decoder input. This framing connects both research directions through one technical theme: the generation, transformation, and use of soft~information.

The role of Chapter 2 is limited to the soft-information interface between learned Demodulation and LDPC decoding. Learned Modulation is relevant because it changes the distribution and calibration of LLRs. The LDPC decoder is then understood as the downstream component whose performance depends on those LLRs. The discussion is therefore system-aware while keeping the LDPC decoder as the center of the thesis.
...
\subsection{Validation Path for Soft-Information Integration}
...
\end{equation}
The present results provide the strongest evidence for $\mathcal{R}_2$ and supporting analysis for the learned-Demodulation side. This statement is precise, defensible, and aligned with the evaluated simulations.

\subsection{Integrated Interpretation for BIBCM-ID-Oriented Receivers}
From a system-level viewpoint, the three chapters form one argument rather than three separate reports. Chapter 1 defines the BIBCM-ID context and explains why Modulation, Demodulation, interleaving, channel decoding, and model-driven learning are coupled through soft information. Chapter 2 analyzes how learned Modulation and Demodulation can affect the reliability values supplied to the LDPC decoder. Chapter 3 then focuses on the main contribution: ANN-assisted LDPC decoding through local check-node message approximation. This ordering first explains the upstream source of decoder input quality and then examines the decoder block itself.
