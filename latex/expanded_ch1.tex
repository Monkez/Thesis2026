...
Several practical receiver issues are important for interpreting the simulation results. First, the LLR sign convention must be used consistently. Some references define $L=\log P(b=0|y)/P(b=1|y)$, while others define the reverse. A sign mismatch between Demodulation and decoding would make the decoder reinforce the wrong hypothesis. The equations in this thesis use one convention consistently within each derivation, and the discussion focuses on relative reliability rather than on a universal sign convention.
...
Third, fixed-point implementation changes message behavior. A floating-point simulation can represent very small probability differences and very large LLR magnitudes. A hardware decoder normally uses quantized messages:
\begin{equation}
