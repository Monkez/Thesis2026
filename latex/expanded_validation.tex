...
\end{equation}
If one decoder reaches this condition earlier than another, it may reduce average complexity even when the maximum iteration limit is the same. Therefore, average iteration count is a useful companion metric:
\begin{equation}
...
An ablation view helps clarify which part of the method is responsible for the observed behavior. The first ablation is SPA imitation versus reliability-aware labeling. If the SPA-imitation network matches SPA, this supports the claim that the MLP can approximate the nonlinear check-node function. If the reliability-aware network improves the curve, this supports the additional claim that learned magnitude control can help under finite loopy decoding. These are two different claims and should not be merged.

The second ablation is network size. A compact network is central to the engineering argument. If a larger network improves BER slightly but greatly increases cost, it may be less relevant to a receiver implementation. Therefore, the hidden-layer size should be interpreted as part of the design trade-off:
\begin{equation}
  \mathrm{Tradeoff}=
...
\end{equation}
where $\psi_\theta$ and $\rho_\theta$ are learnable functions. This form respects the fact that the check-node update should not depend on the arbitrary ordering of incoming edges.
...
The statistical reliability of the reported BER gain depends on simulation length and the number of observed errors. The available curves show a trend under the reported conditions, while lower-BER regions would require longer simulations and more error events for standard-level claims. This interpretation is consistent with a study whose main contribution is algorithmic analysis and feasibility evaluation.
